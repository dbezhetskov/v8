...
uint32_t MemoryChunk::MetadataTableIndex(Address chunk_address, MemoryChunkMetadata* metadata) {
  ...
  if (V8HeapCompressionScheme::GetPtrComprCageBaseAddress(chunk_address) ==
    ...
    index = MemoryChunkConstants::kMainCageMetadataOffset + (offset >> kPageSizeBits);
    ...
    DCHECK_LT(offset >> kPageSizeBits, MemoryChunkConstants::kPagesInTrustedCage);
    index = MemoryChunkConstants::kTrustedSpaceMetadataOffset + (offset >> kPageSizeBits);
  } else {
    ...
    index = MemoryChunkConstants::kCodeRangeMetadataOffset + (offset >> kPageSizeBits);
  }
...
#ifdef V8_COMPRESS_POINTERS_IN_MULTIPLE_CAGES
  return reinterpret_cast<Address>(MemoryChunkMetadataAddrAccess::metadata_pointer_table());
#else
